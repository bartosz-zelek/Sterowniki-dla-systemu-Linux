...
from .state import GFXConnectionState, UARTConnectionState
...
def gfx_console(bp: Builder, name: Namespace, gfx: GFXConnectionState,
                params: GfxConsoleParams):
    title = params.title
    if not title and gfx.gfx_device:
        title = params.console_title
    else:
        title = params.unconnected_title

    gfx.console = bp.obj(name, "graphcon",
        mouse = gfx.mouse,
        keyboard = gfx.keyboard,
        device = gfx.gfx_device,
        abs_mouse = gfx.abs_pointer,
        abs_pointer_enabled = bool(gfx.abs_pointer),
        recorder = name.recorder,
        visible = params.visible,
        screen_size = [params.width, params.height],
        window_title = title,
