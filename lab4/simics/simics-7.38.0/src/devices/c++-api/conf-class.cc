...
ConfClass *ConfClass::add(const iface::InterfaceInfo &iface) {
    ...
    int fail = SIM_register_interface(cls_, iface.name().c_str(),
                                      iface.cstruct());
    if (fail) {
        ...
        SIM_LOG_ERROR(SIM_get_object("sim"), 0,
                      ...
                      iface.name().c_str(), SIM_last_error());
    }
    return this;
}
...
ConfClass *ConfClass::add(EventInfo &&event) {
    if (event.flags_ & Sim_EC_Notsaved) {
        *event.ev_ = SIM_register_event(event.name_.c_str(), *this,
                                        event.flags_,
                                        event.callback_, event.destroy_,
                                        nullptr, nullptr, event.describe_);
    } else {
        *event.ev_ = SIM_register_event(event.name_.c_str(), *this,
                                        event.flags_,
                                        event.callback_, event.destroy_,
                                        event.get_value_, event.set_value_,
                                        event.describe_);
    }

    if (*event.ev_ == nullptr) {
        throw std::runtime_error {
            "Failed to register event " + event.name_
        };
    }
    return this;
}
