...
class FakeObjectFactory : public simics::ObjectFactoryInterface {
  public:
    ...
    ~FakeObjectFactory() override = default;
    ...
    ObjectFactoryInterface *clone() const override {
        clone_called_ = true;
        return new FakeObjectFactory(*this);
    }
...
TEST_F(ConfClassTest, TestAddInterface) {
    FakeObjectFactory object_factory;
    auto conf_class = ConfClass::createInstance("test_add_iface", "short_desc",
                                                "description",
                                                Sim_Class_Kind_Vanilla,
                                                object_factory);
    ...
    const interface_t *iface = reinterpret_cast<const interface_t*>(
                                     uintptr_t{0xdeadbeef});
