...
#include <cci/serialization/boost_unordered_map.hpp>
...
test_unordered_map(){
    ...
    {   
        ...
        oa << cci::serialization::make_nvp("anunorderedmap",anunordered_map);
    }
    boost::unordered_map<random_key, A> anunordered_map1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> cci::serialization::make_nvp("anunorderedmap",anunordered_map1);
    }
...
test_unordered_multimap(){
    ...
    {   
        test_ostream os(testfile, TEST_STREAM_FLAGS);
        test_oarchive oa(os, TEST_ARCHIVE_FLAGS);
        oa << cci::serialization::make_nvp("anunordered_multimap", anunordered_multimap);
    }
    boost::unordered_multimap<random_key, A> anunordered_multimap1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> cci::serialization::make_nvp("anunordered_multimap", anunordered_multimap1);
    }
    std::vector< std::pair<random_key, A> > tvec, tvec1;
    tvec.clear();
    tvec1.clear();
