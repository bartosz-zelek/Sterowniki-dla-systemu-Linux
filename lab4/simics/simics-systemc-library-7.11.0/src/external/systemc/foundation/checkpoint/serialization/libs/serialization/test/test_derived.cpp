...
void save_derived(const char *testfile)
{
    ...
    oa << CCI_SERIALIZATION_NVP(d1) << CCI_SERIALIZATION_NVP(d2);
    ...
    const cci::serialization::extended_type_info & this_type
        = cci::serialization::type_info_implementation<base>::type
            ::get_const_instance();
    ...
    oa << CCI_SERIALIZATION_NVP(b1);
    oa << CCI_SERIALIZATION_NVP(b2);
...
void load_derived(const char *testfile)
{
    ...
    derived1 *d1 = NULL;
    derived2 *d2 = NULL;
    ia >> CCI_SERIALIZATION_NVP(d1) >> CCI_SERIALIZATION_NVP(d2);
    ...
    ia >> CCI_SERIALIZATION_NVP(b1);
    ia >> CCI_SERIALIZATION_NVP(b2);
