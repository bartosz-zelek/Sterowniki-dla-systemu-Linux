...
;; along with this program; see LICENSES-THIRD-PARTY/LICENSE-GPLv2.
;; If not, write to the
...
(defconst c-DML-primitive-type-kwds
  "bool\\|char\\|double\\|float\\|u?int[0-9]*\\|long\\|short\\|void\\|struct\\|union\\|layout\\|bitfields\\|vect")

(defconst c-DML-specifier-kwds
  "local\\|auto\\|const\\|extern\\|static\\|signed\\|unsigned\\|volatile\\|restrict\\|nothrow\\|throws")
...
(defconst c-DML-extra-toplevel-kwds "dml\\|import\\|bitorder\\|loggroup\\|constant\\|header\\|footer\\|typedef")

(defconst c-DML-other-decl-kwds "enum")
...
(defconst dml-font-lock-keywords dml-font-lock-keywords-1
  "Default expressions to highlight in DML editing mode.")
...
\\{c-mode-map}"
  ...
  ;; In Emacs 22, the style is set to the C default, so we need to redo that
  (let ((style (cdr (assq 'dml-mode c-default-style))))
    (when style
      (c-set-style style)))
  ...
  (setq comment-start "// "
	...
	;c-bitfield-key c-C-bitfield-key
        font-lock-defaults `((dml-font-lock-keywords
                             ...
                             nil nil ((?_ . "w") (?- . "w"))
                             ,(if (fboundp 'c-beginning-of-syntax)
                                  ;; This is much faster in Emacs 22
