...
TEST(TestRegisterType, TestRegisterType) {
    simics::register_t r_default;
    auto[name, desc, offset, size, init_val, fields] = r_default;
    EXPECT_TRUE(name.empty());
    EXPECT_TRUE(desc.empty());
    EXPECT_EQ(offset, 0);
    EXPECT_EQ(size, 0);
    EXPECT_EQ(init_val, 0);
    ...
    simics::register_t r_explicit {"register0",
            ...
            {simics::field_t()}};
    std::tie(name, desc, offset, size, init_val, fields) = r_explicit;
