...
class BankPort : public Port<TParent>,
  ...
  public:
    ...
    unsigned number_of_registers() override {
        ...
        return bank_iface_->number_of_registers();
    }
    attr_value_t register_info(unsigned reg) override {
        if (!validate_bank_iface()) {
            return SIM_make_attr_nil();
        }
        auto[reg_offset, reg_iface] = bank_iface_->register_at_index(reg);
        if (reg_iface) {
    ...
    uint64 get_register_value(unsigned reg) override {
        ...
        auto[reg_offset, reg_iface] = bank_iface_->register_at_index(reg);
        return reg_iface ? reg_iface->get() : 0;
    }
    ...
    bool is_read_only(unsigned reg) override {
        ...
        auto[reg_offset, reg_iface] = bank_iface_->register_at_index(reg);
        return reg_iface ? reg_iface->is_read_only() : false;
    }

    // iface::RegisterViewCatalogInterface
    attr_value_t register_names() override {
        size_t number_of_regs = number_of_registers();
        if (number_of_regs == 0) {
            return SIM_make_attr_nil();
        }
        ...
        for (size_t i = 0; i < number_of_regs; ++i) {
            std::tie(std::ignore, iface) = bank_iface_->register_at_index(i);
    ...
    attr_value_t register_offsets() override {
        ...
        size_t reg_offset;
        for (size_t i = 0; i < number_of_regs; ++i) {
            std::tie(reg_offset, std::ignore) = bank_iface_->register_at_index(
                    i);
            SIM_attr_list_set_item(&ret, i, SIM_make_attr_int64(reg_offset));
        }
        return ret;
    }
  ...
  private:
    ...
    static void object_created(lang_void *, conf_object_t *obj) {
        detail::get_interface<BankPortInterface>(obj)->validate_bank_iface();
    }
    ...
    static attr_value_t register_info(size_t address, ByteOrder bo,
        ...
        attr_value_t fields = SIM_alloc_attr_list(fields_info.size());
        unsigned index = 0;
    ...
    void set_bank_name_from_port_name(const std::string &port_name) {
        auto pos = port_name.rfind(bank_name_keyword);
        if (pos == std::string::npos) {
            throw std::invalid_argument(
                    "Invalid bank port name (" + port_name + ")");
        }
        bank_name_ = port_name.substr(pos + bank_name_keyword.size());
    }

    // A keyword exists in the name of all bank ports
    static constexpr std::string_view bank_name_keyword {".bank."};
