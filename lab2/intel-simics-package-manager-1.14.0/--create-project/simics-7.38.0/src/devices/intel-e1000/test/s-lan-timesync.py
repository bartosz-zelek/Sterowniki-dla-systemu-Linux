...
def test_tm_rx(layer = 2, ptp_ver = 2, msg_t = 1):
    ...
    v1_control = 0x11
    v2_msg_id  = 0x22
    src_id = dev_util.tuple_to_value_be(list(map(ord, ['g', 'o', 'o', 'g', 'l', '?'])))
    seq_id = 0xFFFF
    ...
    if ptp_ver == 1:
        ...
        ptp_msg = list(tb.scratch_pad_mem.read(
    ...
    if layer == 2:
        ptp_pkt[12:14] = list(
                dev_util.value_to_tuple_be(PtpConst.ether_type, 2))
        ptp_pkt[14:50] = ptp_msg
    else:
        if msg_t == 1:
            ptp_pkt[36:38] = list(
                    dev_util.value_to_tuple_be(PtpConst.udp_general_port, 2))
        else:
            ptp_pkt[36:38] = list(
                    dev_util.value_to_tuple_be(PtpConst.udp_event_port, 2))
