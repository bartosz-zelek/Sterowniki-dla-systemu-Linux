...
static void
handle_root_modification(sample_linux_mapper_t *mapper, attr_value_t updates)
{
...
static void
handle_added_entities(sample_linux_mapper_t *mapper, attr_value_t added)
{
        add_entities(mapper, added);
}

static void
handle_modified_entities(sample_linux_mapper_t *mapper, attr_value_t updates)
{
        for (int i = 0; i < SIM_attr_dict_size(updates); i++) {
                ...
                if (entity_id == ROOT_ENTITY_ID)
                        ...
                        update_entity(mapper, entity_id, entity_changes);
        }
}

static void
handle_removed_entities(sample_linux_mapper_t *mapper, attr_value_t removed)
{
        remove_entities(mapper, removed);
}

static void
handle_events(sample_linux_mapper_t *mapper, attr_value_t events)
{
        /* This mapper does not support any events. */
}
