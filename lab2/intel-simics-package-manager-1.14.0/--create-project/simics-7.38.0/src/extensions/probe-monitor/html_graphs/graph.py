...
#   This will return an HtmlSection object which data generated for
#   this graph only.
# - When the HtmlSection should be added to the main html-page,
#   the finalize() method should be called.

# A subsection of HTML which is added to the main HtmlPage at
# finalize, when the graph has been produced.
class HtmlSection:
    ...
    def get_new_div(self):
        return self.main_html.get_unique_div_id()
    ...
    def finalize(self):
        # Push our result to main html-page
        self.main_html.add_section(self.html, self.js, self.resize_divs)

class PlotData:
    __slots__ = ('graph_type', 'x_data', 'y_data', 'title',
...
class PlotlyPlot:
    __slots__ = ('main_html', 'plot')

    def __init__(self, main_html, plot):
        self.main_html = main_html
        self.plot = plot
    ...
    def generate(self) -> (HtmlSection):
        html = HtmlSection(self.main_html)
        div = html.get_new_div()
        html.add_resize_divs(div)
        html.add_html(f'<div id="{div}" class="panel"></div>\n')
        ...
        html.add_js("{\n")
        html.add_js(f"  const traces = [{traces_str}];")
        ...
        html.add_js(self._generate_newPlot(
            traces=traces_str,
            div=div,
...
def produce_graph(main_html, plot):
    plotobj = graph_class[plot.graph_type](main_html, plot)
