...
new_command("version", version_cmd,
            ...
            doc = """
Prints a lists of installed Simics products followed by a list of installed
packages with their version numbers. The version for a single package only is
...
def list_script_branch_command(verbose):
...
Lists all currently active script branches. With the <tt>-verbose</tt> flag
            the path for the scripts are included, otherwise just their names. """)
...
def system_info_cmd(name):
    if name:
        try:
            ...
            raise CliError('Failed to open %s: %s' % (name, ex))

        cli_impl.push_stdout(
            outfile, output_mode = cli.output_modes.unformatted_text)

    try:
        system_info_cmd_helper()
    except Exception as ex:
        raise CliError("Failed getting system-info: %s" % ex)
    finally:
        if name:
            cli_impl.pop_stdout()
            outfile.close()
