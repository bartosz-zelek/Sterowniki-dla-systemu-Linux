...
def get_smbus_info(obj):
    ...
    smbus_info = [("SMBus host controller information",
            ...
            "0x%x" % (obj.pci_config_smbus_sram_bar & ~((1 << 8) - 1))),
                     ("SMBus slave device address",
            obj.smbus_func_notify_daddr >> 1),
                    ]),
                 ]
...
def get_smbus_status(obj):
    ...
    is_busy = obj.smbus_func_hst_sts & 0x1
    smbus_status = [("SMBus host controller status",
                      [("Executing",
            enabled_tuple[obj.pci_config_host_configuration & 0x1]),
                       ("SMB interrupting",
            enabled_tuple[(obj.pci_config_host_configuration >> 1) & 0x1]),
                       ("Busy", yes_no_tuple[is_busy]),
                       ("Current SMBus command",
                        ("None",
            smb_cmd_tuple[(obj.smbus_func_hst_cnt >> 2) & 0x7])[is_busy]),
                      ]),
                   ]

    return pci_status + smbus_status
