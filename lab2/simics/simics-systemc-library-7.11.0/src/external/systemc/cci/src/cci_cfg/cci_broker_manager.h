...
class ContextMap {
  public:
    typename T::iterator find(const typename T::key_type &k) {
        T &t = t_[sc_core::sc_curr_simcontext];
        return t.find(k);
    }
    ...
    typename T::mapped_type &operator[] (const typename T::key_type &k) {
        T &t = t_[sc_core::sc_curr_simcontext];
        return t[k];
    }
    void clear() {
        typename std::map<sc_core::sc_simcontext *, T>::iterator i =
            t_.find(sc_core::sc_curr_simcontext);
        sc_assert(i != t_.end());
        i->second.clear();
    }

  private:
    std::map<sc_core::sc_simcontext *, T> t_;
};
...
class cci_broker_manager
{
  ...
  friend void cci_unregister_brokers();
...
private:
    ...
    static ContextMap<std::map<const sc_core::sc_object*, cci_broker_if*> >
        m_brokers;
...
/// Unregister the brokers for the current SystemC simulation
void cci_unregister_brokers();
