...
BOOST_AUTO_TEST_SUITE(TestEthernetCommonExtension_standalone)
  ...
  public:
    explicit TestModule(sc_core::sc_module_name = "TestModule") {
        ...
        dispatcher_.subscribe(
            sif::EthernetCommonExtension::createReceiver(&device_));
    ...
    sif::EthernetCommonExtension extension_;
...
BOOST_FIXTURE_TEST_CASE(testScMain, TestEnvironment) {
    ...
    BOOST_CHECK_EQUAL(consumer.device_.frame_, 0);
    BOOST_CHECK_EQUAL(consumer.device_.frame_crc_ok_, 0);
    simics::types::frags_t frame;
    unsigned char frame_data = 0x79;
    frags_init_add(&frame, &frame_data, 1);

    producer.extension_.frame(&frame, 1);

    BOOST_CHECK_EQUAL(consumer.device_.frame_, 1);
    BOOST_CHECK_EQUAL(consumer.device_.frame_frame_, &frame);
    unsigned char received_data;
    simics::types::frags_extract(consumer.device_.frame_frame_, &received_data);
    BOOST_CHECK_EQUAL(received_data, 0x79);
    BOOST_CHECK_EQUAL(consumer.device_.frame_crc_ok_, 1);
}
