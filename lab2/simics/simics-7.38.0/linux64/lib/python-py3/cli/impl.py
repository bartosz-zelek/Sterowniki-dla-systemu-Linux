...
class filename_t(Arg_type):
    ...
    def normalize_filename(self, original_filename):
        ...
        if self.checkpoint:
            if (self.must_exist
                and not (is_checkpoint_bundle(filename)
                         or (os.path.isfile(filename)
                             and is_config_file(open(filename))))):
...
class Test_get_shortest_unique_object_names(unittest.TestCase):
...
class Test_get_format_string(unittest.TestCase):
...
class Test_new_command(unittest.TestCase):
    def setUp(self):
...
class Test_normalize_value(unittest.TestCase):
    def test_str_t(self):
...
class Test_filename_t_value(unittest.TestCase):
...
def validate(fun_name, arg, value, provided):
    ...
    def normalize_value(handler, v):
        ...
        try:
            ...
            raise ValueError(f"Parameter '{arg.name}', value '{repr(v)}': {e}")
        return ret_val
...
def add_spec(key_spec, name, i, arg, handler):
    ...
    if keyword.iskeyword(key):
        # It is tricky to pass an argument to a keyword-only parameter which
        # name is a Python keyword (see, e.g., the class parameter of
        # the list-objects command). We create an alias for such parameter.
        key_spec[key + '_'] = spec
...
class Test_wrap_cli_cmd_impl(unittest.TestCase):
    def setUp(self):
        self.commands = Command_list()
        self.commands.allow_redefine = False
...
def matches_class_or_iface(object, typename):
    ...
    if object.classname == typename:
        return 1
    if hasattr(object.iface, typename.replace("-", "_")):
        return 1
    return 0
...
class Test_simenv(unittest.TestCase):
...
pre_branch_locals = None
pre_branch_cmdinfo = None

def run_script_branch(cmdinfo, tokens):
    global current_locals, current_cmdinfo
    global pre_branch_locals, pre_branch_cmdinfo
    pre_branch_locals = current_locals
    pre_branch_cmdinfo = current_cmdinfo
    ...
    current_cmdinfo = pre_branch_cmdinfo
...
def start_script_branch(cmdinfo, tokens, desc):
    ...
    return create_branch(lambda: run_script_branch(cmdinfo, tokens),
...
class Test_python_tab_complete(unittest.TestCase):
...
class Test_stack_frame_limit(unittest.TestCase):
...
def cli_sb_wait(command, wait_id, use_obj, caller, filename, line, wait_data):
    global current_locals, current_cmdinfo
    global pre_branch_locals, pre_branch_cmdinfo
    if simics.CORE_is_cli_script_branch():
        branch_locals = current_locals
        branch_cmdinfo = current_cmdinfo
    ...
    try:
        ...
        if simics.CORE_is_cli_script_branch():
            ...
            current_cmdinfo = branch_cmdinfo
...
class Test_print_columns(unittest.TestCase):
...
class Test_print_wrap_code_line(unittest.TestCase):
...
class Test_wrap_code(unittest.TestCase):
    def gives(self, code, width, indent, expect):
        result = ['']
        def output(str):
            result[0] += str
...
class Test_class_function_mape(unittest.TestCase):
    def setUp(self):
...
class Test_common_prefix(unittest.TestCase):
