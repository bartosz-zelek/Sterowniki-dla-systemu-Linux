...
class AfterCall {
  ...
  private:
    // Use a function-local static pointer to the unordered_set per
    // C++ style guideline since STL set has non-trivial destructors
    static std::unordered_set<AfterCallInterface*> &getIfaces() {
        static auto* ifaces = new std::unordered_set<
            AfterCallInterface*>();
        return *ifaces;
    }
};
...
class FunctionCall : public AfterCallInterface {
  public:
    ...
    FunctionCall(const FunctionCall &other)
        : func_(other.func_), name_(other.name_), args_(other.args_) {}
...
class MemberFunctionCall : public AfterCallInterface {
  public:
    ...
    MemberFunctionCall(const MemberFunctionCall& other)
        : func_(other.func_), name_(other.name_), obj_(other.obj_),
          args_(other.args_) {}
    MemberFunctionCall& operator=(const MemberFunctionCall& other) {
        if (this != &other) {
             // not a self-assignment
            func_ = other.func_;
            name_ = other.name_;
