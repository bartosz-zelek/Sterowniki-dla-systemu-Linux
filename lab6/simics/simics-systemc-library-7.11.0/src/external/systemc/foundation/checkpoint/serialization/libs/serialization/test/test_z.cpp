...
#include <cci/serialization/optional.hpp>
...
int main()
{
    ...
    cci::archive::text_oarchive outArchive(outStream);
    outArchive & oldFoo;

    boost::optional<Foo> newFoo;
    std::istringstream inStream(outStream.str());
    cci::archive::text_iarchive inArchive(inStream);
    inArchive & newFoo;
...
int test_main( int /* argc */, char* /* argv */[] )
{
    ...
    const boost::optional<int> aoptional2(123);
    {
        ...
        //oa << cci::serialization::make_nvp("aoptional1",aoptional1);
        //oa << cci::serialization::make_nvp("aoptional2",aoptional2);
    }
    /*
    ...
    boost::optional<int> aoptional2a;
    {
        ...
        ia >> cci::serialization::make_nvp("aoptional1",aoptional1a);
        ia >> cci::serialization::make_nvp("aoptional2",aoptional2a);
    }
...
int test_main(int, char *argv[])
{
	...
	std::wstring w2 = L"апр"; // some non-latin (for example russians) letters
    {
        ...
        {
            cci::archive::xml_woarchive oa(ofs);
            oa << cci::serialization::make_nvp("key1", s1);
            //oa << cci::serialization::make_nvp("key2", w1);
            //oa << cci::serialization::make_nvp("key3", w2); // here exception is thrown
        }
    }
    std::string new_s1;
    //std::wstring new_w1;
    //std::wstring new_w2;
    {
        ...
        {
            cci::archive::xml_wiarchive ia(ifs);
            ia >> cci::serialization::make_nvp("key1", new_s1);
            //ia >> cci::serialization::make_nvp("key2", new_w1);
            //ia >> cci::serialization::make_nvp("key3", new_w2); // here exception is thrown
        }
    }
...
int main() {
        cci::archive::xml_oarchive oa( std::cerr );
        int bob = 3;
        oa << cci::serialization::make_nvp( "bob", bob );
}
...
void go(std::istream &is) {
    ...
    try
    {
        cci::archive::binary_iarchive ia(is);
        ia & vv;
    }
...
namespace bai = cci::archive::iterators;
int main()
{
    std::string input =
