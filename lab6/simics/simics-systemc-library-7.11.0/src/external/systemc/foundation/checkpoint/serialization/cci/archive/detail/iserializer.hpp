#ifndef CCI_ARCHIVE_DETAIL_ISERIALIZER_HPP
#define CCI_ARCHIVE_DETAIL_ISERIALIZER_HPP
...
#ifndef BOOST_MSVC
    #define CCI_DONT_USE_HAS_NEW_OPERATOR (                    \
           BOOST_WORKAROUND(__IBMCPP__, < 1210)            \
        || defined(__SUNPRO_CC) && (__SUNPRO_CC < 0x590)   \
    )
#else
    #define CCI_DONT_USE_HAS_NEW_OPERATOR 0
#endif

#if ! CCI_DONT_USE_HAS_NEW_OPERATOR
...
private:
    virtual void destroy(/*const*/ void *address) const {
        cci::serialization::access::destroy(static_cast<T *>(address));
    }
protected:
...
public:
    ...
    virtual bool tracking(const unsigned int /* flags */) const {
        return cci::serialization::tracking_level< T >::value
                == cci::serialization::track_always
            || ( cci::serialization::tracking_level< T >::value
                == cci::serialization::track_selectively
    ...
    if(file_version > static_cast<const unsigned int>(version()))
        cci::serialization::throw_exception(
            archive::archive_exception(
                cci::archive::archive_exception::unsupported_class_version,
                get_debug_info()
            )
        );
    #endif
...
struct heap_allocation {
    // cci::has_new_operator< T > doesn't work on these compilers
...
) const
{
    ...
    BOOST_TRY {
        ...
        cci::serialization::load_construct_data_adl<Archive, T>(
            ar_impl,
            static_cast<T *>(t),
            file_version
        );
    ...
    ar_impl >> cci::serialization::make_nvp(NULL, * static_cast<T *>(t));
}
...
struct load_pointer_type {
    ...
    static const basic_pointer_iserializer * register_type(Archive &ar, const T & /*t*/){
        ...
        typedef typename
            boost::mpl::eval_if<
                cci::serialization::is_abstract<const T>,
    ...
    find(const cci::serialization::extended_type_info & type){
...
struct load_array_type {
    ...
    static void invoke(Archive &ar, T &t){
        ...
        cci::serialization::collection_size_type count;
        ar >> CCI_SERIALIZATION_NVP(count);
        if(static_cast<std::size_t>(count) > current_count)
            cci::serialization::throw_exception(
                archive::archive_exception(
                    cci::archive::archive_exception::array_size_too_short
                )
            );
...
#endif // CCI_ARCHIVE_DETAIL_ISERIALIZER_HPP
