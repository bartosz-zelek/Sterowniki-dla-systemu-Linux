#ifndef CCI_SERIALIZATION_SLIST_HPP
#define CCI_SERIALIZATION_SLIST_HPP
...
inline void load(
    ...
    if(detail::is_default_constructible<U>()){
        ...
        hint = t.begin();
        while(count-- > 0){
            ar >> cci::serialization::make_nvp("item", *hint++);
        }
    }
    else{
        t.clear();
        cci::serialization::detail::stack_construct<Archive, U> u(ar, item_version);
        ar >> cci::serialization::make_nvp("item", u.reference());
        t.push_front(boost::move(u.reference()));
...
CCI_SERIALIZATION_COLLECTION_TRAITS(BOOST_STD_EXTENSION_NAMESPACE::slist)

#endif  // BOOST_HAS_SLIST
#endif  // CCI_SERIALIZATION_SLIST_HPP
