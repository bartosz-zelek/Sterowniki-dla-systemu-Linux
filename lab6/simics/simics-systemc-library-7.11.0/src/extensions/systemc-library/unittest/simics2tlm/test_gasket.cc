...
BOOST_AUTO_TEST_SUITE(TestSimics2TlmGasket)
...
class TestEnvironment  : public Environment {
  public:
    ...
    void preparePayloadWithDmiAllowed() {
...
class TestDeviceForwardTlm : public TestDevice {
  public:
    virtual void action(tlm::tlm_generic_payload &trans,  // NOLINT
        ...
        BOOST_CHECK_EQUAL(environment_.gasket_->trigger_transaction(), true);
    }
...
class TestDeviceForwardTlmSameEnvironment : public TestDevice {
  public:
    TestDeviceForwardTlmSameEnvironment(
...
BOOST_FIXTURE_TEST_CASE(testTriggerTransaction,
    ...
    BOOST_CHECK_EQUAL(gasket_->trigger_transaction(), true);
    ...
    for (tlm::tlm_response_status *it = errs;
        ...
        BOOST_CHECK_EQUAL(gasket_->trigger_transaction(), false);
...
BOOST_FIXTURE_TEST_CASE(testThrowInDevice, TestEnvironment<TestDeviceThrow>) {
    sc_core::sc_start(sc_core::SC_ZERO_TIME);
    BOOST_CHECK_NO_THROW(gasket_->trigger_transaction());
...
BOOST_FIXTURE_TEST_CASE(testNewTransaction, TestEnvironment<TestDevice>) {
    tlm::tlm_generic_payload &payload = gasket_->payload();

    BOOST_CHECK_EQUAL(payload.get_address(), 0U);
    BOOST_CHECK_EQUAL(payload.get_command(), tlm::TLM_IGNORE_COMMAND);
    BOOST_CHECK(payload.get_data_ptr() == NULL);
    BOOST_CHECK_EQUAL(payload.get_data_length(), 0U);
    BOOST_CHECK_EQUAL(payload.get_response_status(),
                      tlm::TLM_INCOMPLETE_RESPONSE);
    BOOST_CHECK_EQUAL(payload.is_dmi_allowed(), false);
    BOOST_CHECK(payload.get_byte_enable_ptr() == NULL);
    BOOST_CHECK_EQUAL(payload.get_byte_enable_length(), 0U);
    BOOST_CHECK_EQUAL(payload.get_streaming_width(), 0U);
    BOOST_CHECK_EQUAL(payload.get_gp_option(), tlm::TLM_MIN_PAYLOAD);
    BOOST_CHECK_EQUAL(payload.has_mm(), false);
    BOOST_CHECK(payload.get_extension<unittest::MockExtension>() == NULL);
}

BOOST_FIXTURE_TEST_CASE(testClearTransaction, TestEnvironment<TestDevice>) {
    unsigned char data[1] = {};
    unittest::MockExtension extension;
    tlm::tlm_generic_payload &payload = gasket_->payload();

    payload.set_address(1);
    payload.set_read();
    payload.set_data_ptr(data);
    payload.set_data_length(1);
    payload.set_response_status(tlm::TLM_OK_RESPONSE);
    payload.set_dmi_allowed(true);
    payload.set_byte_enable_ptr(data);
    payload.set_byte_enable_length(1);
    payload.set_streaming_width(1);
    payload.set_gp_option(tlm::TLM_FULL_PAYLOAD_ACCEPTED);
    ...
    BOOST_CHECK_EQUAL(payload.get_gp_option(), tlm::TLM_MIN_PAYLOAD);
    BOOST_CHECK_EQUAL(payload.has_mm(), false);
    BOOST_CHECK_EQUAL(payload.get_extension<unittest::MockExtension>(),
                      &extension);

    payload.clear_extension<unittest::MockExtension>();
}
