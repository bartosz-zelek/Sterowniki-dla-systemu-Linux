...
class TestEventEnv : public ::testing::Test {
  public:
...
int TestEventEnv::callback_cnt = 0;
int TestEventEnv::destroy_cnt = 0;
int TestEventEnv::get_value_cnt = 0;
int TestEventEnv::set_value_cnt = 0;

TEST_F(TestEventEnv, TestEventInfo) {
    // Empty name
    ...
    // Null event class
    EXPECT_PRED_THROW(simics::EventInfo("ev", flags, nullptr, callback, destroy,
                                        get_value, set_value, nullptr),
                      std::invalid_argument, checkEmptyEventClass);
    ...
    EXPECT_EQ(ev.name_, "ev");
    EXPECT_EQ(ev.flags_, flags);
    EXPECT_EQ(ev.ev_, &an_event);
    EXPECT_EQ(ev.callback_, callback);
    EXPECT_EQ(ev.destroy_, destroy);
    EXPECT_EQ(ev.get_value_, get_value);
    EXPECT_EQ(ev.set_value_, set_value);
    EXPECT_EQ(ev.describe_, describe);

    ev = simics::EventInfo("ev", &an_event, callback);

    EXPECT_EQ(ev.name_, "ev");
    EXPECT_EQ(ev.flags_, Sim_EC_No_Flags);
    EXPECT_EQ(ev.ev_, &an_event);
    EXPECT_EQ(ev.callback_, callback);
    EXPECT_EQ(ev.destroy_, nullptr);
    EXPECT_EQ(ev.get_value_, nullptr);
    EXPECT_EQ(ev.set_value_, nullptr);
    EXPECT_EQ(ev.describe_, nullptr);
}
...
TEST_F(TestEventEnv, TestSimpleEvent) {
    // Null ConfObject *
...
TEST_F(TestEventEnv, TestCompleteEvent) {
...
TEST_F(TestEventEnv, TestSimpleTimeEvent) {
...
TEST_F(TestEventEnv, TestSimpleCycleEvent) {
...
TEST_F(TestEventEnv, TestSimpleStepEvent) {
