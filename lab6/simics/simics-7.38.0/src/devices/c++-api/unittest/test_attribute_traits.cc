...
TEST(TestAttributeTraits, TestAttrObject) {
    ...
    attr = simics::std_to_attr<>(nil_obj);
...
TEST(TestAttributeTraits, TestAttrString) {
    ...
    attr = simics::std_to_attr<>(empty_str);
...
TEST(TestAttributeTraits, TestAttrBool) {
    ...
    for (bool a_bool : {true, false}) {
        attr = simics::std_to_attr<>(a_bool);
...
TEST(TestAttributeTraits, TestAttrData) {
    ...
    attr = simics::std_to_attr<>(empty_data);
...
TEST(TestAttributeTraits, TestAttrContainer) {
    ...
    // Free the memory allocated in simics::std_to_attr<>
    delete[] attr.private_u.list;
    attr.private_u.list = nullptr;
}
...
TEST(TestAttributeTraits, TestAttrConnectBase) {
    ...
    attr_value_t attr;
    TestConnect connect;
    attr = simics::std_to_attr<>(connect);
    EXPECT_EQ(SIM_attr_is_nil(attr), true);
    EXPECT_EQ(simics::attr_to_std<simics::ConfObjectRef>(attr), nil_obj);
...
TEST(TestAttributeTraits, TestNoExtraCopy) {
    ...
    simics::detail::attr_from_std_helper<decltype(ci_vector)>::f(ci_vector);
    EXPECT_EQ(CountedInt::getCopyConstructorCalls(), 0);
    EXPECT_EQ(CountedInt::getCopyAssignmentCalls(), 0);

    std::list<CountedInt> ci_list(4);
    simics::detail::attr_from_std_helper<decltype(ci_list)>::f(ci_list);
    EXPECT_EQ(CountedInt::getCopyConstructorCalls(), 0);
    EXPECT_EQ(CountedInt::getCopyAssignmentCalls(), 0);

    std::deque<CountedInt> ci_deque(4);
    simics::detail::attr_from_std_helper<decltype(ci_deque)>::f(ci_deque);
    EXPECT_EQ(CountedInt::getCopyConstructorCalls(), 0);
    EXPECT_EQ(CountedInt::getCopyAssignmentCalls(), 0);

    std::pair<int, CountedInt> ci_pair{0, 0};
    simics::detail::attr_from_std_helper<decltype(ci_pair)>::f(ci_pair);
    EXPECT_EQ(CountedInt::getCopyConstructorCalls(), 0);
    EXPECT_EQ(CountedInt::getCopyAssignmentCalls(), 0);

    std::map<int, CountedInt> ci_map;
    ci_map.emplace(0, 4);
    simics::detail::attr_from_std_helper<decltype(ci_map)>::f(ci_map);
    EXPECT_EQ(CountedInt::getCopyConstructorCalls(), 0);
    EXPECT_EQ(CountedInt::getCopyAssignmentCalls(), 0);

    std::tuple<int, CountedInt> ci_tuple {0, 4};
    simics::detail::attr_from_std_helper<decltype(ci_tuple)>::f(ci_tuple);
    EXPECT_EQ(CountedInt::getCopyConstructorCalls(), 0);
    EXPECT_EQ(CountedInt::getCopyAssignmentCalls(), 0);
}
