...
BOOST_AUTO_TEST_SUITE(TestMiiManagementExtension_standalone)
  ...
  public:
    explicit TestModule(sc_core::sc_module_name = "TestModule") {
        ...
        dispatcher_.subscribe(
            sif::MiiManagementExtension::createReceiver(&slave_));
    ...
    sif::MiiManagementExtension extension_;

    // receiver and interface implementation
    MockMiiManagement slave_;
...
BOOST_FIXTURE_TEST_CASE(testScMain, TestEnvironment) {
    ...
    BOOST_CHECK_EQUAL(consumer.slave_.write_register_, 0);
    producer.extension_.write_register(0, 0, 0);
    BOOST_CHECK_EQUAL(consumer.slave_.write_register_, 1);
}
