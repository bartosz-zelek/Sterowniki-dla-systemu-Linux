...
BOOST_AUTO_TEST_SUITE(TestPciDeviceExtension_standalone)
  ...
  public:
    explicit TestModule(sc_core::sc_module_name = "TestModule") {
        ...
        dispatcher_.subscribe(
            sif::PciDeviceExtension::createReceiver(&device_));
    ...
    sif::PciDeviceExtension extension_;
...
BOOST_FIXTURE_TEST_CASE(testScMain, TestEnvironment) {
    ...
    BOOST_CHECK_EQUAL(consumer.device_.bus_reset_, 0);
    producer.extension_.bus_reset();
    BOOST_CHECK_EQUAL(consumer.device_.bus_reset_, 1);
}
