...
#include <cci/serialization/state_saver.hpp>
...
using namespace cci::serialization;
...
basic_iarchive_impl::load_object(
    ...
    cci::serialization::state_saver<object_id_type> ss_start(m_moveable_objects.start);
...
basic_iarchive_impl::load_pointer(
    ...
    if(class_id_type(cobject_info_set.size()) <= cid){
        ...
        || bpis_ptr->get_basic_serializer().is_polymorphic()){
            ...
            char key[CCI_SERIALIZATION_MAX_KEY_SIZE];
...
CCI_ARCHIVE_DECL cci::archive::library_version_type
