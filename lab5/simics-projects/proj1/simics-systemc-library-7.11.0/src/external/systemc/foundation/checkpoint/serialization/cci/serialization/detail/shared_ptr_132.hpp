...
#include <cci/serialization/detail/shared_ptr_nmt_132.hpp>
#else
...
#include <cci/serialization/detail/shared_count_132.hpp>
...
//  except that Borland C++ has a bug, and g++ with -Wsynth warns
    ...
    shared_ptr(shared_ptr<Y> const & r, detail::polymorphic_cast_tag): px(dynamic_cast<element_type *>(r.px)), pn(r.pn)
    {
        if(px == 0)
        {
            cci::serialization::throw_exception(std::bad_cast());
        }
    }
...
// get_pointer() enables cci::mem_fn to recognize shared_ptr
