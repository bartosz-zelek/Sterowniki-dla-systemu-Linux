...
const cci::serialization::extended_type_info & eti(){
    return cci::serialization::singleton<
        cci::serialization::extended_type_info_typeid< T >
    >::get_const_instance();
}
...
test_main( int /* argc */, char* /* argv */[] )
{
    ...
    BOOST_CHECK(vpd == cci::serialization::void_downcast(
        eti<Derived>(),
        eti<Base1>(),
        vpb1
    ));
    BOOST_CHECK(vpb1 == cci::serialization::void_upcast(
        eti<Derived>(),
        eti<Base1>(),
        vpd  
    ));
    ...
    BOOST_CHECK(vpb2 == cci::serialization::void_upcast(
        eti<Derived>(),
        eti<Base2>(),
        vpd
    ));
    BOOST_CHECK(vpmd == cci::serialization::void_downcast(
        eti<MostDerived>(),
        eti<Derived>(),
        vpd
    ));
    BOOST_CHECK(vpd == cci::serialization::void_upcast(
        eti<MostDerived>(),
        eti<Derived>(),
        vpmd
    ));
    ...
    BOOST_CHECK(NULL == cci::serialization::void_downcast(
        eti<Base2>(),
        eti<Base1>(),
        vpb1
    ));
    ...
    cci::serialization::void_cast_register<Derived, Base1>(
        static_cast<Derived *>(NULL),
        static_cast<Base1 *>(NULL)
    );
    cci::serialization::void_cast_register<Derived, Base2>(
        static_cast<Derived *>(NULL),
        static_cast<Base2 *>(NULL)
    );
    cci::serialization::void_cast_register<MostDerived, Derived>(
