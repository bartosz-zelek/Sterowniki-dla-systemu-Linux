#ifndef CCI_ARCHIVE_POLYMORPHIC_IARCHIVE_HPP
#define CCI_ARCHIVE_POLYMORPHIC_IARCHIVE_HPP
...
#include <cci/archive/detail/interface_iarchive.hpp>
...
public:
    ...
    template<class T>
    void load_override(
        const cci::serialization::nvp< T > & t
    ){
        load_start(t.name());
...
public:
    ...
    virtual const detail::basic_pointer_iserializer * load_pointer(
        ...
        const detail::basic_pointer_iserializer * (*finder)(
            const cci::serialization::extended_type_info & type
        )
    ) = 0;
};
...
// required by export
CCI_SERIALIZATION_REGISTER_ARCHIVE(cci::archive::polymorphic_iarchive)

#endif // CCI_ARCHIVE_POLYMORPHIC_IARCHIVE_HPP
