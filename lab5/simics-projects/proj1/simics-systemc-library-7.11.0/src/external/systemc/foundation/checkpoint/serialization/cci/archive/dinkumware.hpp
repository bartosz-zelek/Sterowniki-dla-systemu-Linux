#ifndef CCI_ARCHIVE_DINKUMWARE_HPP
#define CCI_ARCHIVE_DINKUMWARE_HPP
...
operator<<(basic_ostream<CharType> & os, cci::uint64_t t){
...
template<class CharType>
basic_ostream<CharType> & 
operator<<(basic_ostream<CharType> &os, cci::int64_t t){
    if(0 <= t){
        os << static_cast<cci::uint64_t>(t);
    }
    else{
        os.put('-');
        os << -t;
...
operator>>(basic_istream<CharType> &is, cci::int64_t & t){
    CharType d;
    do{
        d = is.get();
    }
...
template<class CharType>
basic_istream<CharType> & 
operator>>(basic_istream<CharType> &is, cci::uint64_t & t){
    cci::int64_t it;
    is >> it;
    t = it;
    return is;
...
#endif //CCI_ARCHIVE_DINKUMWARE_HPP
