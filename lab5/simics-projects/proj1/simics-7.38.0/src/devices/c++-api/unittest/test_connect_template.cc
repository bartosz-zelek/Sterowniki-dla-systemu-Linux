...
TEST(TestConnectTemplate, TestMapTarget) {
    ...
    simics::MapTarget mp{device.obj()};
    ...
    EXPECT_EQ(mp.issue(nullptr, 0x1000), Sim_PE_IO_Not_Taken);
    ...
    transaction_t t;
    EXPECT_EQ(mp.issue(&t, 0x1000), Sim_PE_IO_Not_Taken);
    EXPECT_EQ(Stubs::instance_.sim_log_info_cnt_, ++log_info_cnt_before);
    EXPECT_EQ(Stubs::instance_.SIM_log_info_,
              "map_target not set, transaction terminated");

    simics::ConfObjectRef null_obj {nullptr};
    EXPECT_EQ(mp.set_map_target(null_obj), true);
    EXPECT_EQ(mp.map_target(), nullptr);
    ...
    EXPECT_PRED_THROW(mp.set_map_target(a_obj),
    ...
    Stubs::instance_.new_map_target_ret_ = reinterpret_cast<map_target_t *>(
        0xc0ffee);
    EXPECT_EQ(mp.set_map_target(a_obj), true);
    EXPECT_EQ(mp.map_target(), Stubs::instance_.new_map_target_ret_);
