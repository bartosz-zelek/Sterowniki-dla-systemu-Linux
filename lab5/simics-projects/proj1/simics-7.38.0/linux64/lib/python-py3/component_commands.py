...
def delete_cmd(obj):
    ...
    if SIM_simics_is_running():
        for o in obj.iface.component.get_slot_objects():
            if hasattr(o, 'linkname') or hasattr(o.iface, 'link'):
...
def get_connector_list_cmd(obj, type, c, u):
    if c == u:
        con = None
    else:
        con = bool(c)
...
def save_template(filename):
    ...
    try:
        ...
        for p in map(template_of_object,
            ...
            pobjs[p.name] = p
        try:
            CORE_write_pre_conf_objects(filename, pobjs, 0)
