...
#include <cci/archive/iterators/mb_from_wchar.hpp>
#include <cci/archive/iterators/wchar_from_mb.hpp>
#endif
#include <cci/archive/iterators/xml_escape.hpp>
#include <cci/archive/iterators/xml_unescape.hpp>
#include <cci/archive/iterators/transform_width.hpp>
#include <cci/archive/iterators/istream_iterator.hpp>
#include <cci/archive/iterators/ostream_iterator.hpp>
...
void test_wchar_from_mb(const wchar_t *la, const char * a, const unsigned int size){
    typedef cci::archive::iterators::wchar_from_mb<const char *> translator;
    BOOST_CHECK((
        std::equal(
            translator(a),
            translator(a + size),
...
void test_mb_from_wchar(const char * a, const wchar_t *la, const unsigned int size){
    typedef cci::archive::iterators::mb_from_wchar<const wchar_t *> translator;
    BOOST_CHECK(
        std::equal(
            translator(la),
            translator(la + size),
...
void test_roundtrip(const wchar_t * la){
    ...
    {
        typedef cci::archive::iterators::wchar_from_mb<std::vector<char>::const_iterator> translator;
        std::copy(
            translator(a.begin()),
            translator(),
...
void test_xml_escape(
    ...
    typedef cci::archive::iterators::xml_escape<const CharType *> translator;
...
void test_xml_unescape(
    ...
    // test xml_unescape
    typedef cci::archive::iterators::xml_unescape<const CharType *> translator;
...
void test_transform_width(unsigned int size){
    ...
    typedef cci::archive::iterators::transform_width<
        char *, BitsOut, BitsIn 
    > translator1;
...
void test_stream_iterators(
    ...
    cci::archive::iterators::ostream_iterator<CharType> osi =
        cci::archive::iterators::ostream_iterator<CharType>(ss);
    std::copy(test_data, test_data + size, osi);

    BOOST_CHECK(size == ss.str().size());

    cci::archive::iterators::istream_iterator<CharType> isi =
        cci::archive::iterators::istream_iterator<CharType>(ss);
