...
test_map(){
    ...
    {   
        ...
        oa << cci::serialization::make_nvp("amap", amap);
    }
    std::map<random_key, A> amap1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> cci::serialization::make_nvp("amap", amap1);
    }
...
test_map_2(){
    ...
    b[1] = 1;
    {
        ...
        test_oarchive oa(os, TEST_ARCHIVE_FLAGS);
        oa << CCI_SERIALIZATION_NVP(pb);
        oa << CCI_SERIALIZATION_NVP(pa);
    }
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        ...
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> CCI_SERIALIZATION_NVP(pb);
        ia >> CCI_SERIALIZATION_NVP(pa);
        delete pa;
        delete pb;
    }
    std::remove(testfile);
...
test_multimap(){
    ...
    {   
        test_ostream os(testfile, TEST_STREAM_FLAGS);
        test_oarchive oa(os, TEST_ARCHIVE_FLAGS);
        oa << cci::serialization::make_nvp("amultimap", amultimap);
    }
    std::multimap<random_key, A> amultimap1;
    {
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive ia(is, TEST_ARCHIVE_FLAGS);
        ia >> cci::serialization::make_nvp("amultimap", amultimap1);
    }
