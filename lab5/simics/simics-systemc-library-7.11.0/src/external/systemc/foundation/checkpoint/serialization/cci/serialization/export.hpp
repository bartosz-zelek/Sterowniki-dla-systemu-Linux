#ifndef CCI_SERIALIZATION_EXPORT_HPP
#define CCI_SERIALIZATION_EXPORT_HPP
...
#include <cci/serialization/extended_type_info.hpp> // for guid_defined only
...
struct guid_initializer
{
    ...
    guid_initializer const & export_guid() const {
        ...
        //CCI_STATIC_WARNING(! cci::serialization::is_abstract< T >::value);
        export_guid(cci::serialization::is_abstract< T >());
        return *this;
    }
};
...
#define CCI_CLASS_EXPORT_IMPLEMENT(T)                      \
    namespace cci {                                        \
    namespace archive {                                      \
    namespace detail {                                       \
    namespace extra_detail {                                 \
...
#define CCI_CLASS_EXPORT_KEY2(T, K)          \
namespace cci {                              \
...
#define CCI_CLASS_EXPORT_KEY(T)                                      \
    CCI_CLASS_EXPORT_KEY2(T, BOOST_PP_STRINGIZE(T))                                                                  \
/**/

#define CCI_CLASS_EXPORT_GUID(T, K)                                  \
CCI_CLASS_EXPORT_KEY2(T, K)                                          \
CCI_CLASS_EXPORT_IMPLEMENT(T)                                        \
/**/
...
namespace {                                                                    \
  ...
  (::cci::archive::detail::instantiate_ptr_serialization((Derived*)0,0), 3); \
  static int BOOST_PP_CAT(boost_serialization_mwerks_init2_, __LINE__) = (     \
      ::cci::serialization::void_cast_register((Derived*)0,(Base*)0)         \
    , 3);                                                                      \
}
...
// need to export it.
#define CCI_CLASS_EXPORT_CHECK(T)                              \
    CCI_STATIC_WARNING(                                        \
...
#define CCI_CLASS_EXPORT(T)                   \
    CCI_CLASS_EXPORT_GUID(                    \
        T,                                      \
        BOOST_PP_STRINGIZE(T)                   \
    )                                           \
    /**/

#endif // CCI_SERIALIZATION_EXPORT_HPP
