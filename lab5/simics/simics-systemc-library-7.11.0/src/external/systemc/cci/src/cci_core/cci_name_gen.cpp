...
cci_get_cci_unique_names()
{
    ...
    static std::map<sc_core::sc_simcontext *,
                    std::map<std::string, std::pair<int, cci_name_state> > >
                cci_unique_names;
    return cci_unique_names[sc_core::sc_curr_simcontext];
}
#endif
...
#else
    // Erase the name from the map. A newly allocated context may end up using
    // a previously deallocated context's address, as is the case in our
    // unittests.
    if (cci_get_cci_unique_names().erase(name)) {
        return true;
    } else {
        return false;
    }
