...
BOOST_AUTO_TEST_SUITE(TestPciOperationUpstreamExtension_standalone)
  ...
  public:
    explicit TestModule(sc_core::sc_module_name = "TestModule") {
        ...
        dispatcher_.subscribe(
            sif::PciUpstreamOperationExtension::createReceiver(&device_));
    ...
    sif::PciUpstreamOperationExtension extension_;
...
BOOST_FIXTURE_TEST_CASE(testScMain, TestEnvironment) {
    ...
    BOOST_CHECK_EQUAL(consumer.device_.read_, 0);
    producer.extension_.read(8, simics::types::Sim_Addr_Space_Memory);
    BOOST_CHECK_EQUAL(consumer.device_.read_, 1);
    BOOST_CHECK_EQUAL(consumer.device_.rid_, 8);
    BOOST_CHECK_EQUAL(consumer.device_.addr_space_,
