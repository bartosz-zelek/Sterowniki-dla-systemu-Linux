...
BOOST_AUTO_TEST_SUITE(TestSerialDeviceExtension_standalone)
  ...
  public:
    explicit TestModule(sc_core::sc_module_name = "TestModule") {
        ...
        dispatcher_.subscribe(
            sif::SerialDeviceExtension::createReceiver(&device_));
    ...
    sif::SerialDeviceExtension extension_;
...
BOOST_FIXTURE_TEST_CASE(testScMain, TestEnvironment) {
    ...
    BOOST_CHECK_EQUAL(consumer.device_.write_, 0);
    producer.extension_.write(0);
    BOOST_CHECK_EQUAL(consumer.device_.write_, 1);
}
