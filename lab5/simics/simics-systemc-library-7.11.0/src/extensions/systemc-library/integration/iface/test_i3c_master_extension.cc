...
BOOST_AUTO_TEST_SUITE(TestI3cMasterExtension_standalone)
  ...
  public:
    explicit TestModule(sc_core::sc_module_name = "TestModule") {
        ...
        dispatcher_.subscribe(
            sif::I3cMasterExtension::createReceiver(&master_));
    ...
    sif::I3cMasterExtension extension_;

    // receiver and interface implementation
    MockI3cMaster master_;
...
BOOST_FIXTURE_TEST_CASE(testScMain, TestEnvironment) {
    ...
    BOOST_CHECK_EQUAL(consumer.master_.acknowledge_, 0);
    producer.extension_.acknowledge(simics::types::I3C_ack);
    BOOST_CHECK_EQUAL(consumer.master_.acknowledge_, 1);
}
