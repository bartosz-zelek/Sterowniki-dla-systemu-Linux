...
def cmd_dev_reg_list_common(obj, bspec, pattern, substr, byname, show_description):
    ...
    if pattern != "*" and substr:
        output += "Note: pattern and substr both set. Ignoring substr.\n"
        substr = None
    if substr:
        pattern = f"*{glob.escape(substr)}*"
    for bank in banks:
        (b_output, b_regs) = print_reg_list(
    ...
    return cli.command_verbose_return(output, regs)
...
def print_reg_list(obj, bank, pattern, byname, show_description):
    regs = []
    bname = bank.name if bank.name else ""
    for rname in bank.reg_names():
        ...
        assert r, rname
        data = [r.offset, bname, r.name, r.size, read_reg_val(
            obj, bank, r)]
        if show_description:
            description = ''
            if bank.rviface:
    ...
    outfile = StringIO()
    def fmt_none_or_num(value):
        return 'N/A' if value is None else cli.number_str(value)

    if byname:
        fmt = 'lrrr'
        header = ['Name', 'Offset', 'Size', 'Value']
        sort_key = itemgetter(2)
    else:
        fmt = 'rlrr'
        header = ['Offset', 'Name', 'Size', 'Value']
    ...
    data = []
    for r in sorted(regs, key=sort_key):
        name = r[2]
        offset = fmt_none_or_num(r[0])
        size = r[3]
        value = fmt_none_or_num(r[4])
        description = [r[5]] if show_description else []
        if byname:
            data.append([name, offset, size, value] + description)
        else:
            data.append([offset, name, size, value] + description)
    cli.print_columns(fmt, [header] + data, outfile=outfile)
    return (outfile.getvalue(), sorted([x[:5] for x in regs]))
