...
/*
  Ensure that a cache line is flushed from the STCs
*/
void
          ...
          cache_line_status_t status)
{
        ...
        if (SIM_clear_exception()) {
                SIM_LOG_ERROR(&gc->obj, GC_Log_Cache,
        ...
        if (gc->timing_model) {
                ...
                if (gc->config.virtual_tag) {
                        SIM_set_mem_op_virtual_address(
                                &mt.s, GC_UNTAG(gc, cl->tag));
                        SIM_set_mem_op_physical_address(
                                &mt.s, GC_UNTAG(gc, cl->otag));
                }
                else {
                        SIM_set_mem_op_physical_address(
                                &mt.s, GC_UNTAG(gc, cl->tag));
                ...
                mt.s.may_stall = 1;
                SIM_set_mem_op_initiator(&mt.s,
                                         Sim_Initiator_Cache,
                                         &gc->obj);
                SIM_set_mem_op_exception(&mt.s, Sim_PE_No_Exception);
                ...
                if (SIM_get_mem_op_exception(&mt.s) != Sim_PE_No_Exception)
                        SIM_set_mem_op_exception(
                                mem_op, SIM_get_mem_op_exception(&mt.s));
        }
           ...
           conf_object_t *space, map_list_t *map)
{
        ...
        if (gc->timing_model) {
                ...
                mt.s.size = gc->config.line_size;
                mt.s.may_stall = 1;
                SIM_set_mem_op_initiator(&mt.s, Sim_Initiator_Cache, &gc->obj);
                SIM_set_mem_op_exception(&mt.s, Sim_PE_No_Exception);
                ...
                if (SIM_get_mem_op_exception(&mt.s) != Sim_PE_No_Exception)
                        SIM_set_mem_op_exception(
                                mem_op,
                                SIM_get_mem_op_exception(&mt.s));
        }

        /* if the cache is non-write allocate, the write STC may contain a copy
           of this line so we need to flush it to ensure correct MESI and line
           status */
...
lookup_line(generic_cache_t *gc, generic_transaction_t *mem_op)
{
        ...
        for (i = index * gc->config.assoc; i < (index + 1) * gc->config.assoc; i++)
...
mesi_send(generic_cache_t *gc, generic_transaction_t *mem_op)
{
        ...
        SIM_LOG_INFO(4, &gc->obj, GC_Log_MESI, "mesi_send");
                ...
                int line_num)
{
        ...
        case CL_Exclusive:
                SIM_LOG_INFO(
                        3, &gc->obj, GC_Log_MESI,
                        "mesi_receive: snoop read hit "
                        "on an exclusive line");
                
                /* flush STC to ensure that a write hit will change the line
                   status */
                if (!gc->config.no_STC)
                        flush_STC(gc, 
...
mesi_snoop_transaction(generic_cache_t *gc, generic_transaction_t *mem_op)
{
        ...
        orig_pa = SIM_get_mem_op_physical_address(mem_op);
        orig_la = SIM_get_mem_op_virtual_address(mem_op);

        for (size = 0;
             size < SIM_get_mem_op_size(mem_op);
             size += gc->config.line_size) {

                SIM_set_mem_op_physical_address(mem_op, orig_pa + size);
                SIM_set_mem_op_virtual_address(mem_op, orig_la + size);
                line_num = lookup_line(gc, mem_op);
                SIM_LOG_INFO(4, &gc->obj, GC_Log_MESI, "mesi_receive");
                ...
                if (line_num != -1) {
                        ...
                        if (SIM_mem_op_is_read(mem_op)) {
                                ...
                                SIM_LOG_INFO(3, &gc->obj, GC_Log_MESI,
        ...
        SIM_set_mem_op_physical_address(mem_op, orig_pa);
        SIM_set_mem_op_virtual_address(mem_op, orig_la);
            ...
            conf_object_t *space, map_list_t *map)
{
        ...
        if (line_num != -1) {
                /* Yes, we have a hit. */
                SIM_LOG_INFO(3, &gc->obj, GC_Log_Read_Hit,
                ...
                /* No, we have a miss. */
                SIM_LOG_INFO(3, &gc->obj, GC_Log_Read_Miss,
                ...
                if (SIM_mem_op_is_data(mem_op)) {
                        ...
                        if (gc->prof[I_DRM].id)
                                VT_prof_data_inc_pc(gc->prof[I_DRM].id,
                                                    GC_CPU(mem_op));
        ...
        if (HOOK_BLOCK_STC(
                    gc, line_num, 
                    gc->config.no_STC 
                    || ((SIM_get_mem_op_ini_type(mem_op) & 0xFF00)
                        == Sim_Initiator_Cache))) {
                SIM_LOG_INFO(4, &gc->obj, GC_Log_Cache, 
             ...
             conf_object_t *space, map_list_t *map)
{
        ...
        if (line_num != -1) {
                /* Yes, we have a hit. */
                SIM_LOG_INFO(3, &gc->obj, GC_Log_Write_Hit,
                ...
                /* No, we have a miss. */
                SIM_LOG_INFO(3, &gc->obj, GC_Log_Write_Miss,
                ...
                if (gc->prof[I_DWM].id) {
                        ...
                        if (!gc->config.write_allocate)
                                mem_op->block_STC = 1;
                        VT_prof_data_inc_pc(gc->prof[I_DWM].id, GC_CPU(mem_op));
                }
           ...
           map_list_t *map, generic_transaction_t *mem_op)
{
        ...
        if (SIM_mem_op_is_prefetch(mem_op)) {
                ...
                if (is_uncacheable(gc, mem_op, map, &gc->obj) 
                    || HOOK_DISABLED(gc)) {                                 

                        /* Handle uncacheable transactions. */
                        SIM_LOG_INFO(3, &gc->obj, GC_Log_Cache,
                        ...
                        SIM_LOG_INFO(1, &gc->obj, GC_Log_Cache,
        ...
        if (stall_time != 0) {
                if (SIM_mem_op_may_stall(mem_op)) {
                        SIM_LOG_INFO(4, &gc->obj, GC_Log_Cache,
                                     ...
                                     stall_time);
                        return stall_time;
                } else if (SIM_mem_op_is_from_cpu(mem_op)) {
                        SIM_LOG_INFO(4, &gc->obj, GC_Log_Cache,
                                     "operate: posting %lld cycles penalty",
                                     stall_time);
                        conf_object_t *i_obj = SIM_get_mem_op_initiator(mem_op);
                        /* In case there's already some stall registered, add
                           our stall cycles. */
                        cycles_t count = SIM_stalled_until(i_obj) + stall_time;
                        SIM_stall_cycle(i_obj, count);
                        return 0;
                } else {
             ...
             cycles_t (*fun)(generic_cache_t *, generic_transaction_t *))
{
        ...
        if (((SIM_get_mem_op_physical_address(mem_op)
              ^ (SIM_get_mem_op_physical_address(mem_op)
                 + SIM_get_mem_op_size(mem_op) - 1))
             >> gc->config.line_size_log2) == 0)
                return fun(gc, mem_op);

        /* Create a wrapper memop. */
        trans.s = *mem_op;
        SIM_set_mem_op_initiator(&trans.s,
                                 Sim_Initiator_Cache,
                                 cache);
        if ((SIM_get_mem_op_ini_type(mem_op) & 0xFF00)
            == Sim_Initiator_Cache) {
        ...
        offs = 0;
        while (offs < SIM_get_mem_op_size(mem_op)) {
                SIM_set_mem_op_physical_address(
                        &trans.s,
                        SIM_get_mem_op_physical_address(mem_op) + offs);
                SIM_set_mem_op_virtual_address(
                        &trans.s,
                        SIM_get_mem_op_virtual_address(mem_op) + offs);
                start = SIM_get_mem_op_physical_address(&trans.s);
                end = MIN(start | (gc->config.line_size - 1),
                          SIM_get_mem_op_size(mem_op) - offs);
