...
def do_test(hash_func, queue, processor):
    ...
    # Prepare the receive buffer in the memory
    off = RX_BUF_BASE
    for i in range(RX_BUF_LEN // SIM_MEM_BLOCK_LEN):
        tb.write_mem(off, tuple([0x00 for j in range(SIM_MEM_BLOCK_LEN)]))
        off += SIM_MEM_BLOCK_LEN

    # Prepare the extended receive descriptor(read format) in the memory
    off = RX_DESC_BASE
    for i in range(RX_DESC_LEN // SIM_MEM_BLOCK_LEN):
        tb.write_mem(off, tuple([0x00 for j in range(SIM_MEM_BLOCK_LEN)]))
        off += SIM_MEM_BLOCK_LEN
    ...
    for i in range(rd_cnt):
        tb.adv_rd_read_layout.word0.PACKET_BUFFER = rbuf_addr
        tb.adv_rd_read_layout.word1.HEADER_BUFFER = 0
        tb.write_mem(rd_addr,
            tuple(tb.scratch_pad_mem.read(SCRATCH_EX_RD_BASE, ICH9_LAN_RD_LEN)))
        rbuf_addr += BASE_PKT_BUF_SIZE
        rd_addr += ICH9_LAN_RD_LEN

    # Configure the receive descriptor ring of destined queue
    tb.write_reg("RDBAL%d" % queue, RX_DESC_BASE)
    tb.write_reg("RDBAH%d" % queue, 0)
    tb.write_reg("RDLEN%d" % queue, rd_cnt * ICH9_LAN_RD_LEN)
    tb.write_reg("RDH%d" % queue, 0)
    tb.write_reg("RDT%d" % queue, rd_cnt - 1)

    # Configure the MAC address of this LAN
    mac_addr = dev_util.tuple_to_value_le(tuple(TestData.tcp_pkt[0:6]))
    rah_val = IchLanConst.rah_bf.value(RAH = mac_addr >> 32, AV = 1)
    tb.write_reg("RAL0", mac_addr & ((1 << 32) - 1))
    tb.write_reg("RAH0", rah_val)
    ...
    for para_i in range(para_cnt):
        ...
        tb.write_reg("SRRCTL%d" % queue, IchLanConst.srrctl_bf.value(
                BSIZEPACKET = 2, # Kbytes
                BSIZEHEADER = 4, # 4*64 = 256
                DESCTYPE = 1     # One buffer
                ))
        ...
        tb.scratch_pad_mem.write(SCRATCH_EX_RD_BASE,
                                 tb.read_mem(rd_addr, ICH9_LAN_RD_LEN))
        rsshash = (tb.adv_rd_layout.word0.PKTCSUM << 16) | tb.adv_rd_layout.word0.IPID
        expect_hex(rsshash, hash_val, "RSS hash value")
        expect(tb.adv_rd_layout.word0.RSS_TYPE, hash_func + 1, "RSS hash function")
