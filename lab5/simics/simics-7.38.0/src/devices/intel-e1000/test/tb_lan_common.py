...
class IchLanDriver:
    ...
    def reset_mac(self):
        ...
        self.tb.lan.pci_config_command = 0x4  # BME

    # Configure the MAC address
    def config_mac_addr(self, queue, addr_tuple):
        mac_addr = dev_util.tuple_to_value_le(addr_tuple)
    ...
    def config_lstx_smac_addr_pi(self, addr_tuple, pi_val):
        mac_addr = dev_util.tuple_to_value_be(addr_tuple)
...
class TestBench:
    def __init__(self, instantiate_lan_device_func):
        self.spi_flashes = []
        self.flash_images = []
        ...
        # Pseudo interrupt pin
        pseudo_dev = dev_util.Dev([('intr_pin', dev_util.Signal)])
        self.pdev = pseudo_dev.obj
        self.intr_pin = pseudo_dev.intr_pin.signal
        ...
        self.scratch_pad_mem = dev_util.Memory()
    ...
    def prepare_tx_packet(self, tx_pkt, tbuf_addr = TX_BUF_BASE,
        ...
        if legacy_td == 0:
            ...
            self.write_mem(next_td,
                tuple(self.scratch_pad_mem.read(SCRATCH_C_TD_BASE, ICH9_LAN_C_TD_LEN)))
        ...
        if legacy_td:
            ...
            self.write_mem(next_td,
                tuple(self.scratch_pad_mem.read(SCRATCH_L_TD_BASE, ICH9_LAN_L_TD_LEN)))
            ...
            self.write_mem(next_td,
                tuple(self.scratch_pad_mem.read(SCRATCH_D_TD_BASE, ICH9_LAN_D_TD_LEN)))
...
#                           IchLanConst.tidv_bf.value(IDV = delay_cnt, FDP = 1))
        ...
        mac_addr = dev_util.tuple_to_value_le(tuple(tx_pkt[6:12]))
        rah_val = IchLanConst.rah_bf.value(RAH = mac_addr >> 32, AV = 1)
        ...
        for i in range(rd_cnt):
            if split_rd:
                ...
                for j in range(buf_cnt):
                    self.scratch_pad_mem.write(
                        pad_addr,
                        dev_util.value_to_tuple_le(buf_addr, 8))
                    pad_addr += 8
                    buf_addr += (buf_size << 10)
                self.write_mem(rd_addr + i * SCRATCH_SRD_LEN,
                               tuple(self.scratch_pad_mem.read(
    ...
    def adv_prepare_to_rx_packet(self, rbuf_addr = RX_BUF_BASE,
        ...
        self.adv_rd_layout.clear()
        if desc_type == 1:
            self.scratch_pad_mem.write(pad_addr,
                                       dev_util.value_to_tuple_le(head_addr, 8))
        else:
            self.scratch_pad_mem.write(pad_addr,
                                       dev_util.value_to_tuple_le(buf_addr, 8))
            self.scratch_pad_mem.write(pad_addr + 8,
                                       dev_util.value_to_tuple_le(head_addr, 8))

        self.write_mem(rd_addr, tuple(self.scratch_pad_mem.read(
