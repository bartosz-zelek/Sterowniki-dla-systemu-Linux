...
TEST(TestAfter, TestFunctionCallNoArgument) {
    ...
    fc.set_args(simics::std_to_attr<std::tuple<>>(std::make_tuple()));
    EXPECT_THROW(
            fc.set_args(simics::std_to_attr<std::tuple<int>>(2)),
            simics::detail::SetIllegalType);

    auto *new_fc = fc.make_copy();
...
TEST(TestAfter, TestFunctionCallOneIntArgument) {
    ...
    fc.set_args(simics::std_to_attr<std::tuple<int>>(std::make_tuple(0x1234)));
    EXPECT_THROW(
            fc.set_args(simics::std_to_attr<std::tuple<bool>>(true)),
            simics::detail::SetIllegalType);
...
TEST(TestAfter, TestFunctionCallOneStrArgument) {
    ...
    fc.set_args(simics::std_to_attr<std::tuple<std::string>>(
...
TEST(TestAfter, TestFunctionCallTwoArguments) {
    ...
    fc.set_args(simics::std_to_attr<std::tuple<std::vector<float>, int>>(
...
TEST(TestAfter, TestMemberFunctionCallNoArgument) {
    ...
    mfc.set_args(simics::std_to_attr<
                 std::tuple<simics::ConfObjectRef>>(
                     std::make_tuple(t.obj())));
    EXPECT_THROW(
        mfc.set_args(simics::std_to_attr<std::tuple<simics::ConfObjectRef,
                     int>>(std::make_tuple(t.obj(), 2))),
        simics::detail::SetIllegalType);

    auto *new_mfc = mfc.make_copy();
...
TEST(TestAfter, TestAfterEvent) {
    ...
    auto attr1 = simics::std_to_attr<std::tuple<>>(std::tuple<>());
    EXPECT_PRED_THROW(ae.set_value(attr1), std::invalid_argument,
                      checkAfterEventSetValue);
    auto attr2 = simics::std_to_attr<std::pair<int, int>>({2, 3});
    EXPECT_PRED_THROW(ae.set_value(attr2), std::invalid_argument,
                      checkAfterEventSetValue);
    auto attr3 = simics::std_to_attr<std::pair<std::string, int>>({"2", 3});
    EXPECT_PRED_THROW(ae.set_value(attr3), std::invalid_argument,
                      checkAfterEventSetValue);
    auto attr4 = simics::std_to_attr<std::pair<std::string, std::tuple<>>>(
        {"foo", std::tuple<>()});
...
std::vector<int> test_member_function_call3_arg {};
...
TEST(TestAfter, TestEnableAfterEvent) {
    ...
    ev.cancel_all();
    EXPECT_EQ(Stubs::instance_.event_cancel_time_obj_, obj);
