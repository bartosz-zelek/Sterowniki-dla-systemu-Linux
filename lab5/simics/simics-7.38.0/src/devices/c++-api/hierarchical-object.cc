...
void HierarchicalObject::init_iface(BankInterface *iface) {
    if (level_ != Level::BANK) {
        std::string err = "Bank name (" + hierarchical_name_ \
            + ") does not match the bank level (bankA)";
        SIM_LOG_CRITICAL_STR(bank_obj_ref(), 0, err);
        throw std::invalid_argument { err };
    }
    dev_obj_->set_iface<BankInterface>(hierarchical_name_, iface);
}

void HierarchicalObject::init_iface(RegisterInterface *iface) {
    if (level_ != Level::REGISTER) {
        std::string err = "Register name (" + hierarchical_name_ \
            + ") does not match the register level (bankA.registerB)";
        SIM_LOG_CRITICAL_STR(bank_obj_ref(), 0, err);
        throw std::invalid_argument { err };
    }
    dev_obj_->set_iface<RegisterInterface>(hierarchical_name_, iface);
}

void HierarchicalObject::init_iface(FieldInterface *iface) {
    if (level_ != Level::FIELD) {
        std::string err = "Field name (" + hierarchical_name_ \
            + ") does not match the field level (bankA.registerB.fieldC)";
        SIM_LOG_CRITICAL_STR(bank_obj_ref(), 0, err);
        throw std::invalid_argument { err };
    }
    dev_obj_->set_iface<FieldInterface>(hierarchical_name_, iface);
}
...
void HierarchicalObject::ensureBankPortExists(const std::string &bank_name) {
    if (bank_name.empty()) {
        throw std::invalid_argument {
            "Empty bank name is not allowed"
        };
    }
