...
#ifndef SYSTEMC_CHECKPOINT_SERIALIZATION_STACK_H
#define SYSTEMC_CHECKPOINT_SERIALIZATION_STACK_H
...
// deque, list, and vector are valid containers to be used by the stack adapter
...
template <class Archive, class T, class Container>
void load(Archive &archive, std::stack<T, Container> &s,  // NOLINT
          const unsigned int version) {
    load(archive, sc_checkpoint::containerOf(s), version);
}

template <class Archive, class T, class Container>
void serialize(Archive &archive, std::stack<T, Container> &s,  // NOLINT
               const unsigned int version) {
...
#endif  // SYSTEMC_CHECKPOINT_SERIALIZATION_STACK_H
