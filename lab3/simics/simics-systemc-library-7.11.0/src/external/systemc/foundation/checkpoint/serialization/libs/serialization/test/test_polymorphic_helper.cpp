...
CCI_SERIALIZATION_SPLIT_FREE(my_string)
...
int test_main(int /* argc */, char * /* argv */ [])
{
    ...
    // test using using polymorphic implementation.
    {
        ...
        oa_implementation << cci::serialization::make_nvp("vector", v1);
    }
    {
        std::vector<my_string> v2;
        test_istream is(testfile, TEST_STREAM_FLAGS);
        test_iarchive  ia_implementation(is, TEST_ARCHIVE_FLAGS);
        ia_implementation >> cci::serialization::make_nvp("vector", v2);
        BOOST_CHECK(v1 == v2);
    }
    std::remove(testfile);
    ...
    // test using using polymorphic interface.
    {
        ...
        oa_interface << cci::serialization::make_nvp("vector", v1);
    }
    {
        std::vector<my_string> v2;
        test_istream is(testfile, TEST_STREAM_FLAGS);
        ...
        ia_interface >> cci::serialization::make_nvp("vector", v2);
        BOOST_CHECK(v1 == v2);
    }
    std::remove(testfile);
    std::remove(testfile);
