...
/*
 * Change: Conditional verbosity check added to optimize performance.
 * This modification prevents unnecessary string construction when
 * the current verbosity level is below the specified threshold.
 * The change is backported from kernel 3.0.0 and wrapped with #if INTC_EXT
 * to allow easy toggling between the original and modified behavior.
 */
#if INTC_EXT
