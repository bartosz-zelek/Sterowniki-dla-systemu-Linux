...
BOOST_AUTO_TEST_SUITE(TestPciBusExtension_standalone)
  ...
  public:
    explicit TestModule(sc_core::sc_module_name = "TestModule") {
        ...
        dispatcher_.subscribe(
            sif::PciBusExtension::createReceiver(&bus_));
    ...
    sif::PciBusExtension extension_;
...
BOOST_FIXTURE_TEST_CASE(testScMain, TestEnvironment) {
    ...
    BOOST_CHECK_EQUAL(consumer.bus_.raise_interrupt_, 0);
    producer.extension_.raise_interrupt(0);
    BOOST_CHECK_EQUAL(consumer.bus_.raise_interrupt_, 1);
}
