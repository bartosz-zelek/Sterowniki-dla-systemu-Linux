// -*- mode: C++; c-file-style: "virtutech-c++" -*-

/*
  © 2014 Intel Corporation

  This software and the related documents are Intel copyrighted materials, and
  your use of them is governed by the express license under which they were
  provided to you ("License"). Unless the License provides otherwise, you may
  not use, modify, copy, publish, distribute, disclose or transmit this software
  or the related documents without Intel's prior written permission.

  This software and the related documents are provided as is, with no express or
  implied warranties, other than those that are expressly stated in the License.
*/

#ifndef SIMICS_SYSTEMC_CONTEXT_H
#define SIMICS_SYSTEMC_CONTEXT_H

#include <systemc>

#include <simics/systemc/iface/simulation_interface.h>
#include <simics/systemc/kernel.h>
#include <simics/systemc/report_handler.h>

namespace simics {
namespace systemc {

/** Utility class that handles the context switching, using RAII methodology.
 *
 * Also provides an overloaded '->' operator for accessing the underlying
 * sc_simcontext.
 */
class Context {
  public:
    explicit Context(const iface::SimulationInterface *sim);
    virtual ~Context();
    sc_core::sc_simcontext *operator->() const;

  private:
    Kernel kernel_;
    ReportHandler report_handler_;
};

}  // namespace systemc
}  // namespace simics

#endif
