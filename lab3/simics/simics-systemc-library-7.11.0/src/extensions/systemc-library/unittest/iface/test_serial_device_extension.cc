...
#include "mock/iface/mock_serial_device.h"
#include "mock/simics2tlm/mock_gasket.h"

BOOST_AUTO_TEST_SUITE(TestSerialDeviceExtension)
...
class TestEnvironment {
  public:
    TestEnvironment() {
        ...
        receiver_
            = simics::systemc::iface::SerialDeviceExtension::createReceiver(
                &device_);
    ...
    MockSerialDevice device_;
    simics::systemc::iface::SerialDeviceExtension extension_;
    simics::systemc::iface::ReceiverInterface *receiver_;
};

BOOST_FIXTURE_TEST_CASE(testWrite, TestEnvironment) {
    device_.write_return_ = 1;
    extension_.write(5);
    BOOST_CHECK_EQUAL(device_.write_, 1);
    BOOST_CHECK_EQUAL(device_.write_value_, 5);
}

BOOST_FIXTURE_TEST_CASE(testReceiveReady, TestEnvironment) {
    extension_.receive_ready();
    BOOST_CHECK_EQUAL(device_.receive_ready_, 1);
}
