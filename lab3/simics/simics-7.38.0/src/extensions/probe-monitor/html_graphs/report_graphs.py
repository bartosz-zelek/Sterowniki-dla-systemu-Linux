...
class HtmlGraphs:
    __slots__ = ('title', 'tooltip', 'html_sections')
    def __init__(self, title, tooltip, html_sections):
        self.title = title
...
def produce_session_graphs(main_page, session, graph_specification):
    ...
    for g in graph_specification["graphs"]:
        ...
        if html_sections:
            sections.append(HtmlGraphs(title, tooltip, html_sections))
