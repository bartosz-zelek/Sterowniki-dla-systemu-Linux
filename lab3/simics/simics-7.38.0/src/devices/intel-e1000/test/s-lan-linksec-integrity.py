...
# s-lan-linksec-integrity.py
# tests the LinkSec(MACsec) integrity check in RX/TX

# Tested data flow:
#   packet -> upper protocol -> TX -> LinkSec integrity value added
#   -> PHY -> RX -> LinkSec integrity value checked -> packet
...
def do_test(key, pkt, sa, an, pn, pi):
    ...
    lan_drv.config_lstx_mode(IchLanConst.LSEC_TX_MODE_ADD_INTG)
    ...
    # Check the output ethernet frame in the PHY
    ...
    tb.scratch_pad_mem.write(SCRATCH_MACSEC_SECTAG_BASE, recv_pkt[12:28])
    ...
    expect(tb.sectag_layout.TCI_AN.E,  0, "SecTAG encrypted")
    expect(tb.sectag_layout.TCI_AN.C,  0, "SecTAG changed text")
    ...
    expect_list(dec_pkt, pkt, "integrity checked packet")
